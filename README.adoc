...
. The diff between `actual` and `old` revisions will be stored; Note that `old` revision can be provided in plugin configuration using `previousRevision` field. If no revision is provided then previous repository revision will be used.
+
