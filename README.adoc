...
. While your job runs the plugin reads your build workspace to generate VCS diffs;
. The diff between `actual` and `previous` revisions will be stored on your build;
. The `diff` for each build can be viewed later in html format.

NOTE: To get most from this plugin use `periodically SCM pooling` to trigger your builds, http://www.nailedtothex.org/roller/kyle/entry/articles-jenkins-poll[more details here^].
