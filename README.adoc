...
. The diff between `actual` and `previous` revisions will be stored; Note that `previous` revision can be provided in plugin configuration using `previousRevision` field. If no revision is provided then previous repository revision will be used.
+
TIP: You can use https://wiki.jenkins.io/display/JENKINS/Parameterized+Build[parameters] is previous revision. In case of git, expressions like `HEAD^{tree}` or `HEAD^^` can be used.
...
Only *previousRevision* and *sinceLastSuccessfulBuild* options are not related to diff2html. The first expects a revision id to create the diff and the second, when checked, will set previous revision to last successful build revision.
